...
//            sc: SparkContext,
//            save: Boolean = false): (GnocchiModel, RDD[Association[T]]) = {
//
//    // call RegressPhenotypes on the data
//    val assocs = fit[T](rdd, phenotypes)
//
//    // extract the model parameters (including p-value) for each variant and build LogisticGnocchiModel
//    val model = extractModel[T](assocs, sc)
//
//    (model, assocs)
//  }
//
//  def fit[T](rdd: RDD[GenotypeState],
//          phenotypes: RDD[Phenotype]): RDD[Association[T]]
//
//  def extractModel[T](assocs: RDD[Association[T]], sc: SparkContext): GnocchiModel
//
//}
//
...
//            phenotypes: RDD[Phenotype],
//            sc: SparkContext): (GnocchiModel, RDD[Association[T]]) = {
//    val assocs = fit[T](genotypeStates, phenotypes)
//    (extractModel[T](assocs, sc), assocs)
//  }
//
//  def fit[T](rdd: RDD[GenotypeState],
//          phenotypes: RDD[Phenotype]): RDD[Association[T]] = {
