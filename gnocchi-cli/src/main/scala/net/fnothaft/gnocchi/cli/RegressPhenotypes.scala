...
class RegressPhenotypes(protected val args: RegressPhenotypesArgs) extends BDGSparkCommand[RegressPhenotypesArgs] {
  ...
  def run(sc: SparkContext) {
    ...
    val phenotypes = sc.loadPhenotypes(args.phenotypes, args.phenoName, args.oneTwo, args.includeCovariates, args.covarFile, args.covarNames)
  ...
  def performAnalysis(genotypeStates: RDD[GenotypeState],
    ...
    val associations = args.associationType match {
      case "ADDITIVE_LINEAR"   => AdditiveLinearRegression(genotypeStates, phenotypes)
      case "DOMINANT_LINEAR"   => DominantLinearRegression(genotypeStates, phenotypes)
      case "ADDITIVE_LOGISTIC" => AdditiveLogisticRegression(genotypeStates, phenotypes)
      case "DOMINANT_LOGISTIC" => DominantLogisticRegression(genotypeStates, phenotypes)
    }
