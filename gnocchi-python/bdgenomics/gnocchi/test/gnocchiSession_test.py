...
class GnocchiSessionTest(SparkTestCase):
    ...
    def test_load_genotypes(self):
        ...
        self.assertEqual(genotypes._jvmDS.genotypes().toJavaRDD().count(), 1)
    ...
    def test_load_phenotypes(self):
        ...
        self.assertEqual(phenoMap._jvmPhenoContainer.phenotypes().value().size(), 5)
    ...
    def test_filters_integration(self):
        ...
        self.assertEqual(genotypes._jvmDS.genotypes().toJavaRDD().count(), 10000)

        genotypes = gs.filterSamples(genotypes, 0.1, 2)
        genotypes = gs.filterVariants(genotypes, 0.1, 0.1)

        self.assertEqual(genotypes._jvmDS.genotypes().toJavaRDD().count(), 8232)
